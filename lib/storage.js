...
(function (root) {
  ...
  // Serialize every read-modify-write so concurrent callers (e.g. rapid
  // context-menu saves while a previous saveClip is mid-flight) can't clobber
  // each other. Each public mutation runs inside withLock; their internal
  // getClips calls go through _getClipsInternal to avoid re-entering the lock.
  ...
  function getClips() {
    return withLock(_getClipsInternal);
  }
  ...
  function saveClip(input, opts) {
    ...
    return _getClipsInternal().then(function (clips) {
